...
from config.path_config import EmbeddingPaths, NodePairPaths, OutputPaths
...
parser.add_argument('--alpha-s', required=True, help='Value for alpha_s')
parser.add_argument('--alpha-c', required=True, help='Value for alpha_c')
...
within_node_u_LINE = read_single_graph_line_embeddings(EmbeddingPaths.LINE.format('subgraph1', alpha_s, alpha_c, args.embedding_size))
within_node_v_LINE = read_single_graph_line_embeddings(EmbeddingPaths.LINE.format('subgraph2', alpha_s, alpha_c, args.embedding_size))


embedding_path = ''
if args.embedding == 'LINE':
    embedding_path = EmbeddingPaths.LINE
elif args.embedding == 'deepWalk':
    embedding_path = EmbeddingPaths.deepWalk
elif args.embedding == 'node2vec':
    embedding_path = EmbeddingPaths.node2vec
across_node_u_LINE, across_node_v_LINE = read_combined_line_embeddings(embedding_path.format('combined', alpha_s, alpha_c, args.embedding_size))
...
seed_list = read_nodepairs(NodePairPaths.fb_nodepairs.format(alpha_s, alpha_c, type='seed'))
nonseed_list = read_nodepairs(NodePairPaths.fb_nodepairs.format(alpha_s, alpha_c, type='nonseed'))
...
print('alpha_s: {}, alpha_c: {}, embedding: {}'.format(alpha_s, alpha_c, args.embedding))
...
out_file = OutputPaths.accuracy_file.format(args.embedding, alpha_s, alpha_c, args.embedding_size)
